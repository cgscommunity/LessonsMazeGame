...
//used only in this file so it's fine to live here
...
GameplayState::GameplayState(StateMachineExampleGame* pOwner)
	: m_pOwner(pOwner)
	//not descriptive, bool should answer question
	//can use enum if there is more than one state
	, m_beatLevel(false)
	, m_skipFrameCount(0)
	, m_currentLevel(0)
	, m_pLevel(nullptr)
...
bool GameplayState::Load()
{
	...
	m_pLevel = new Level();
	//make a struct to represent position
...
//huge function
//single threaded, blocked on getch
//also cover code reviews
bool GameplayState::Update(bool processInput)
{
	//this is stricly to process input
	...
	//boolean processInput is strictly for the below
	//can be called seperately to avoid if check
	if (m_beatLevel)
	{
		++m_skipFrameCount;
...
{
	//not clear that a loop to update all actors occurs here
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		case ActorType::Door:
		{
			//how is this handled, the function that opens a door should still live on the door
			//and it should update it's data
			//need to read up on ecs
			Door* collidedDoor = dynamic_cast<Door*>(collidedActor);
			assert(collidedDoor);
			//could be a door system, and takes id and handles it
			if (!collidedDoor->IsOpen())
			{
				if (m_player.HasKey(collidedDoor->GetColor()))
				{
					...
					m_player.UseKey();
					m_player.SetPosition(newPlayerX, newPlayerY);
					//this is an issue with singleton, code will be scattered with instance
